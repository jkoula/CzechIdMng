...
- 🟠 [#3158](https://redmine.czechidm.com/issues/3158) - Version 13.0.0 introduces possibility to assign role to other entities than identity contracts. This change brings few challenges for developers:
  ...
  - Developers should review all code, which is using `IdmIdentityRole`, `IdmIdentityRoleService`, `IdmConceptRoleRequest` and `IdmConceptRoleRequestService` (along with their respective Dtos) and decide, whether it should be abstracted to include all role assignment types, or whether it is a feature specific to `IdmIdentityRole`, or `IdmConceptRoleRequest`.
  - Neglecting this step should not break any existing behavior, but may lead to unexpected result when dealing with other role assignment types.
...
- 🟠 [#3036](https://redmine.czechidm.com/issues/3036) - Configuration od IdmDbAppender must be changed. If you are using database logging appender, replace the following section in your logback-spring.xml files
