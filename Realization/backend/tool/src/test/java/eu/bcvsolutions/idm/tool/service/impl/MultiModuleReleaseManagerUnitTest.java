	...
	protected String prepareRepository() {
		...
		try {
			...
			FileUtils.writeStringToFile(new File(backendFolder.getPath() +"/" + ModuleReleaseManager.IDM_PREFIX_BACKEND + ROOT_MODULE_ID + "/pom.xml"),
					...
					+ "<modules>"
						+ "<module>" + ModuleReleaseManager.IDM_PREFIX_BACKEND + ROOT_MODULE_ID + "-api</module>"
						+ "<module>" + ModuleReleaseManager.IDM_PREFIX_BACKEND + ROOT_MODULE_ID + "-impl</module>"
					+ "</modules>" +
					"</project>",
					AttachableEntity.DEFAULT_CHARSET);
			// api
			FileUtils.writeStringToFile(new File(backendFolder.getPath() +"/" + ModuleReleaseManager.IDM_PREFIX_BACKEND + ROOT_MODULE_ID
					+ "/" + ModuleReleaseManager.IDM_PREFIX_BACKEND + ROOT_MODULE_ID + "-api/pom.xml"),
					"<project>"
					+ "<modelVersion>4.0.0</modelVersion>"
					+ "<parent>"
					  ...
					  + "<version>" + version + "</version>"
					+ "</parent>"
					+ "<artifactId>" + ModuleReleaseManager.IDM_PREFIX_BACKEND + ROOT_MODULE_ID + "-api</artifactId>"
