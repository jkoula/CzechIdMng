...
----- TABLE acc_role_assignment_form_value -----
CREATE TABLE acc_role_assignment_form_value
(
    id bytea NOT NULL,
    ...
    uuid_value bytea,
    CONSTRAINT acc_role_assignment_form_value_pkey PRIMARY KEY (id),
    CONSTRAINT acc_role_assignment_form_value_seq_check CHECK (seq <= 99999)
);

CREATE INDEX idx_acc_role_assignment_form
    ON acc_role_assignment_form_value
    USING btree
    (owner_id);

CREATE INDEX idx_acc_role_assignment_form_def
    ON acc_role_assignment_form_value
    USING btree
    (attribute_id);

CREATE INDEX idx_acc_role_assignment_form_stxt
    ON acc_role_assignment_form_value
    USING btree
    (short_text_value);

CREATE INDEX idx_acc_role_assignment_form_uuid
    ON acc_role_assignment_form_value
    USING btree
    (uuid_value);


----- TABLE acc_role_assignment_form_value_a -----
CREATE TABLE acc_role_assignment_form_value_a
(
    id bytea NOT NULL,
    rev bigint NOT NULL,
    revtype smallint,
    ...
    CONSTRAINT acc_role_assignment_form_value_a_pkey PRIMARY KEY (id, rev),
    CONSTRAINT fk_acc_role_assignment_form_value_a FOREIGN KEY (rev)
