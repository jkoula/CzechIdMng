...
public class DefaultIdmRoleRequestServiceIntegrationTest extends AbstractCoreWorkflowIntegrationTest {
	...
	public void addPermissionViaRoleRequestTest() {
		...
		request.setApplicantInfo(new ApplicantImplDto(testA.getId(), IdmIdentityDto.class.getCanonicalName()));
	...
	public void testExecuteRequestForSameAutomaticRole() {
		...
		requestOne.setApplicantInfo(new ApplicantImplDto(identity.getId(), IdmIdentityDto.class.getCanonicalName()));
		...
		requestTwo.setApplicantInfo(new ApplicantImplDto(identity.getId(), IdmIdentityDto.class.getCanonicalName()));
	...
	@Test
	public void testExecuteRequestForSameAutomaticRoleWithBusinessRole() {
		IdmRoleDto role = getHelper().createRole();
		IdmRoleDto roleSub = getHelper().createRole();
		IdmRoleDto roleSubSub = getHelper().createRole();

		getHelper().createRoleComposition(role, roleSub);
		getHelper().createRoleComposition(roleSub, roleSubSub);
		...
		Set<UUID> allRoles = Stream.of(role, roleSub, roleSubSub).map(IdmRoleDto::getId).collect(Collectors.toSet());
		Set<UUID> assignedRolesIds = assignedRoles.stream().map(IdmIdentityRoleDto::getRole).collect(Collectors.toSet());

		Assert.assertEquals(allRoles, assignedRolesIds);
	}
