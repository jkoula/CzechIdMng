...
/**
 * Permissions to incompatible roles by sub role.
 * 
 * @author Radek Tomiška
 * @since 9.4.0
 */
@Component(IncompatibleRoleBySubRoleEvaluator.EVALUATOR_NAME)
@Description("Permissions to incompatible roles by sub role.")
public class IncompatibleRoleBySubRoleEvaluator extends AbstractTransitiveEvaluator<IdmIncompatibleRole> {

	public static final String EVALUATOR_NAME = "core-incompatible-role-by-sub-role-evaluator";
	...
	public Predicate getPredicate(Root<IdmIncompatibleRole> root, CriteriaQuery<?> query, CriteriaBuilder builder, AuthorizationPolicy policy, BasePermission... permission) {
		...
		subquery.where(builder.and(
				...
				builder.equal(root.get(IdmIncompatibleRole_.sub), subRoot) // correlation attribute
				));
		//
