...
/**
 * Permissions to assigned role-form-attributes by role.
 * 
 * @author Vít Švanda
 * @since 9.4.0
 */
@Component(RoleFormAttributeByRoleEvaluator.EVALUATOR_NAME)
@Description("Permissions to assigned role-form-attributes by role")
public class RoleFormAttributeByRoleEvaluator extends AbstractTransitiveEvaluator<IdmRoleFormAttribute> {

	public static final String EVALUATOR_NAME = "core-role-form-attribute-by-role-evaluator";
	...
	@Override
	protected Identifiable getOwner(IdmRoleFormAttribute entity) {
	...
	@Override
	public Predicate getPredicate(Root<IdmRoleFormAttribute> root, CriteriaQuery<?> query, CriteriaBuilder builder, AuthorizationPolicy policy, BasePermission... permission) {
		...
		subquery.where(builder.and(
				...
				builder.equal(root.get(IdmRoleFormAttribute_.role), subRoot) // correlation attribute
				));
		//
	...
	@Override
	public Set<String> getPermissions(IdmRoleFormAttribute entity, AuthorizationPolicy policy) {
