...
/**
 * First step of Group MS AD connector.
 *
 * @author Vít Švanda
 * @since 11.1.0
 */
export default class StepOne extends AbstractWizardStep {
  ...
  render() {
    const {connectorType} = this.props;
    const {showLoading, sslSwitch, identityMappingSearchParameters, memberSystemMappingId, systemName} = this.state;

    const _connectorType = this.state.connectorType ? this.state.connectorType : connectorType;
    const formData = {};
    if (_connectorType && _connectorType.metadata) {
      const metadata = _connectorType.metadata;
      formData.name = systemName || metadata.name;
    ...
    return (
      <Basic.Div showLoading={showLoading}>
        <Basic.AbstractForm
          ...
          <Basic.TextField
            ref="name"
            onChange={this._onChangeSystemName.bind(this)}
