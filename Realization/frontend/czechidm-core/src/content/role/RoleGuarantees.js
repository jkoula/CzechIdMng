...
export default class RoleGuarantees extends Basic.AbstractContent {
  componentDidMount() {
    ...
    this.context.store.dispatch(
      codeListManager.fetchCodeListIfNeeded("guarantee-type")
    );
  }
  ...
  render() {
    const forceSearchParameters = new SearchParameters().setFilter(
      "role",
      ...
      <Basic.Div>
        ...
        {!SecurityManager.hasAuthority("ROLEGUARANTEE_READ") || (
          <Basic.Div>
            <Basic.ContentHeader
              icon="fa:group"
              text={this.i18n("identity.header")}
              style={{ marginBottom: 0 }}/>
            <RoleGuaranteeTable
